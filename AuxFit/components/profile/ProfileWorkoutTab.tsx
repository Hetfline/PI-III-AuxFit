    ...
    try {
        ...
        if (Array.isArray(history)) {
            ...
            const formatted = history
                .filter((h: any) => h.volume_total !== null && Number(h.volume_total) > 0)
                .map((h: any) => ({
    ...
    <View style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statCard}>
          ...
          <Text style={[Texts.title, styles.statValue]}>{lastVolume.toLocaleString('pt-BR')}</Text>
          <Text style={[Texts.subtext, { fontSize: 12 }]}>kg totais</Text>
        </View>

        <View style={styles.statCard}>
          <Text style={[Texts.subtext, styles.statLabel]}>Média</Text>
          <Text style={[Texts.title, styles.statValue]}>{avgVolume.toLocaleString('pt-BR')}</Text>
          <Text style={[Texts.subtext, { fontSize: 12 }]}>kg / treino</Text>
        </View>

        <View style={styles.statCard}>
          <Text style={[Texts.subtext, styles.statLabel]}>Recorde</Text>
          <Text style={[Texts.title, styles.statValue, { color: Colors.primary }]}>
        ...
        <View style={styles.chartContainer}>
          {progressData.length === 0 ? (
             <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
                <Text style={Texts.subtext}>Nenhum dado de treino encontrado.</Text>
                <Text style={[Texts.subtext, {fontSize: 12, marginTop: 4}]}>Finalize um treino para ver seu progresso!</Text>
             </View>
          ) : (
            <>
            {/* Eixo Y Simplificado */}
            <View style={styles.yAxis}>
                <Text style={styles.axisLabel}>{maxVolume > 1000 ? (maxVolume/1000).toFixed(1)+'k' : maxVolume}</Text>
                <Text style={styles.axisLabel}>{minVolume > 1000 ? (minVolume/1000).toFixed(1)+'k' : minVolume}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  statCard: { flex: 1, backgroundColor: Colors.bgMedium, borderRadius: 12, padding: Spacing.sm, alignItems: 'center' },
  statLabel: { fontSize: 12, textAlign: 'center', marginBottom: 4 },
  statValue: { fontSize: 18, marginBottom: 2 },
  chartSection: { backgroundColor: Colors.bgMedium, borderRadius: 16, padding: Spacing.md, marginBottom: Spacing.md },
  chartContainer: { flexDirection: 'row', height: 180 },
  yAxis: { width: 40, justifyContent: 'space-between', paddingVertical: 20, paddingRight: 8 },
  axisLabel: { ...Texts.subtext, fontSize: 10, textAlign: 'right' },
  barsContainer: { flex: 1, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', gap: 8, paddingBottom: 20 },
  barWrapper: { flex: 1, alignItems: 'center', height: '100%', justifyContent: 'flex-end' },
  barContainer: { width: '100%', height: '100%', justifyContent: 'flex-end', alignItems: 'center' },
  bar: { width: '80%', borderTopLeftRadius: 4, borderTopRightRadius: 4 },
  barLabel: { ...Texts.subtext, fontSize: 10, marginTop: 6, textAlign: 'center' },
});