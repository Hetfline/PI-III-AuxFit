...
const STYLE_OPTIONS: OptionItem[] = [
  {
    ...
    description: 'Foco em ganho de massa e força. Séries com descanso controlado (Musculação clássica).',
    icon: 'arm-flex',
    color: '#4CAF50'
  },
  {
    ...
    description: 'Alta frequência cardíaca, circuitos e pouco descanso. Foco em queima de gordura.',
    icon: 'run-fast',
    color: '#FF5722'
  },
  {
    ...
    description: 'Combina força bruta (cargas altas) com acessórios de hipertrofia.',
    icon: 'weight-lifter',
    color: '#2196F3'
  }
];
...
export default function StyleScreen() {
  ...
  const handleNext = () => {
    ...
    // Próxima tela: Lesões / Limitações
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        <Background />
        <Toast message="Selecione um estilo." visible={showToast} onHide={() => setShowToast(false)} type="warning" />
        
        {/* Pergunta 5 de 7 */}
        <ProgressBar currentQuestion={5} totalQuestions={7} onBack={() => router.replace("/onboarding/workout/experienceScreen")} />

        <View style={styles.content}>
          <View style={styles.topSection}>
            <Text style={styles.question}>Qual estilo de treino você prefere?</Text>
            <Text style={styles.subtitle}>Isso define a metodologia (séries retas, bi-sets, circuitos, etc).</Text>
          </View>
