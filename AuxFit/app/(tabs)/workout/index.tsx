...
export default function WorkoutScreen() {
  ...
  const handleSaveWorkout = async (data: { nome: string; duracao: number; areas_foco: string[] }) => {
    try {
      if (editingWorkout) {
  ...
  const filteredExercises = exercises.filter(e => 
    selectedFocusAreas.length === 0 || selectedFocusAreas.includes(e.grupo_muscular_geral)
  ).filter(e => {
    if (!searchTerm) return true;
    const search = searchTerm.toLowerCase();
    return e.nome_exercicio.toLowerCase().includes(search) || 
        ...
        {planos.map((plano) => {
          ...
          return (
            <Pressable
              ...
              style={[
                styles.planCard,
                { backgroundColor: isSelected ? Colors.primary : Colors.bgLight },
              ]}
            >
              <Text style={[Texts.bodyBold, { color: isSelected ? Colors.bg : Colors.text }]}>
                {plano.name}
              </Text>
            </Pressable>
          );
  ...
  const renderTreinoItem = ({ item }: { item: Workout }) => {
    ...
    const focusString = item.areas_foco && item.areas_foco.length > 0 
        ...
        <View style={{ flex: 1 }}>
          ...
          {activeTab === "treino" ? (
            <FlatList
              ...
              ListEmptyComponent={() => (
                !loadingWorkouts && (
                  <Text style={[Texts.body, { textAlign: 'center', color: Colors.subtext, marginVertical: 20 }]}>
          ...
          >
            <View>
              <Text style={Texts.bodyBold}>
                Áreas de foco <Text style={[Texts.subtext, { color: Colors.accent }]}>
...
const styles = StyleSheet.create({
  ...
  planCard: { borderRadius: 10, padding: Spacing.sm, backgroundColor: Colors.bgLight, alignItems: "center" },
  planCardContainer: { flexDirection: "row", overflow: "scroll", gap: Spacing.sm },
});